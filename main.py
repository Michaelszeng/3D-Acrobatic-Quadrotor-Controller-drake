...
# pose_goal = np.array([2.5, 1.5, 2, 3.14, 0, 3.14])
pose_goal = np.array([0, 2, 3, 1.57, 0, 0])
...
"""
(visual_quadrotor_model_instance1,) = parser.AddModelsFromString(urdf, ".urdf")
...
<robot name="visual_quadrotor{i}">
  ...
  <link name="visual_quadrotor_base_link">
    ...
    <visual>
      <origin rpy="1.570796 0 0" xyz="0 0 0"/>
      <geometry>
        <mesh filename="file://{os.path.join(script_dir, 'skydio_2_1000_poly.obj')}" scale=".00254"/>
      </geometry>
    </visual>
  </link>
</robot>
...
plant.SetFreeBodyPose(plant_context, plant.GetBodyByName("base_link1", visual_quadrotor_model_instance1), visual_quadrotor_pose)
visual_quadrotor_joint_idx = plant.GetJointIndices(visual_quadrotor_model_instance1)[0]
...
for i in range(N):
  if 12 <= i <= 19 and i != 15 :
    pose = RigidTransform(RotationMatrix(x_trj[i,6:15].reshape(3,3)), [100, 100, 100])
  else:
    pose = RigidTransform(RotationMatrix(x_trj[i,6:15].reshape(3,3)), x_trj[i,:3])
  plant.SetFreeBodyPose(plant_context, plant.GetBodyByName("visual_quadrotor_base_link", quad_instances[i]), pose)
  visual_quadrotor_joint_idx = plant.GetJointIndices(quad_instances[i])[0]
  visual_quadrotor_joint = plant.get_joint(visual_quadrotor_joint_idx)  # Joint object
  visual_quadrotor_joint.Lock(plant_context)
...
def counter_to_rgba(counter, num_trj):
    ...
    if num_trj == 0:
        return (1, 0, 0, 1)  # Red, if there are no steps in the trajectory
...
for x_trj in all_x_trj:
  ...
  meshcat.SetLine(f"ddp traj {ctr}", pos_3d_matrix, rgba=Rgba(rgba[0], rgba[1], rgba[2], rgba[3]))
  ctr+=1
